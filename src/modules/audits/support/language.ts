...
import { countSyllables, countWords } from '../../../utils/strings.js';
import { QUESTION_HEADING_PATTERN, AI_CRAWLERS, MODIFIED_DATE_SELECTORS, PUBLISH_DATE_SELECTORS } from './patterns.js';
...
export function extractEntities(text: string): ExtractedEntities {
  ...
  const people = [...new Set(doc.people().out('array') as string[])].slice(0, 10);
  const organizations = [...new Set(doc.organizations().out('array') as string[])].slice(0, 10);
  const places = [...new Set(doc.places().out('array') as string[])].slice(0, 10);
  const topics = [...new Set(doc.topics().out('array') as string[])].slice(0, 15);
...
export function evaluateFreshness($: CheerioAPI): FreshnessResult {
...
// Gap 3: Section Length Analysis
export interface SectionLengthResult {
  sectionCount: number;
  avgWordsPerSection: number;
  sections: number[];
}

export function measureSectionLengths($: CheerioAPI): SectionLengthResult {
    ...
    for (const line of lines) {
      ...
      if (lower.startsWith('user-agent:')) {
        ...
        if (lower.startsWith('disallow:')) {
          ...
          if (path === '/') {
            if (currentAgent === crawlerLower) { isBlocked = true; found = true; }
            else if (currentAgent === '*' && !found) { isBlocked = true; }
          }
        } else if (lower.startsWith('allow:')) {
          if (currentAgent === crawlerLower) { found = true; isBlocked = false; }
        }
      }
    }
