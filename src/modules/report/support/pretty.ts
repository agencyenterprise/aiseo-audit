...
export function renderPretty(result: AnalyzerResult): string {
  const lines: string[] = [];
  const width = 60;
  ...
  lines.push(
    chalk.dim(`  Points: ${result.totalPoints}/${result.maxPoints}`)
  );
  lines.push('');
  ...
  for (const category of Object.values(result.categories)) {
    ...
    const catPct = category.maxScore > 0
  ...
  if (result.recommendations.length > 0) {
    ...
    if (result.recommendations.length > 8) {
      lines.push(
        chalk.dim(`  ... and ${result.recommendations.length - 8} more recommendations`)
      );
    }
  }
  ...
  lines.push(divider);
  lines.push(
    chalk.dim(`  Analyzed at: ${result.analyzedAt}`)
  );
  lines.push(
    chalk.dim(`  Duration: ${result.meta.analysisDurationMs}ms`)
  );
  lines.push(divider);
  lines.push('');
