...
export function renderMarkdown(result: AnalyzerResult): string {
  ...
  for (const category of Object.values(result.categories)) {
    const pct = category.maxScore > 0
      ...
      : 0;
    lines.push(`| ${category.name} | ${category.score} | ${category.maxScore} | ${pct}% |`);
  }
  ...
  for (const category of Object.values(result.categories)) {
    ...
    for (const factor of category.factors) {
      const statusIcon = factor.status === 'good' ? 'pass'
        : factor.status === 'neutral' ? '-'
        : factor.status === 'needs_improvement' ? 'warn'
        : 'fail';
      lines.push(`| ${factor.name} | ${factor.score}/${factor.maxScore} | ${statusIcon} | ${factor.value} |`);
    }
  ...
  if (result.recommendations.length > 0) {
    ...
    for (const rec of result.recommendations) {
      const tag = rec.priority === 'high' ? '**HIGH**'
        : rec.priority === 'medium' ? '*MEDIUM*'
        : 'LOW';
      lines.push(`- [${tag}] **${rec.factor}** (${rec.category}): ${rec.recommendation}`);
    }
  ...
  lines.push('---');
  lines.push(`*Generated by generative-engine-audit-cli v${result.meta.version}*`);
