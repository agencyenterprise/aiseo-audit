...
**Boilerplate Ratio** is computed by stripping `<script>`, `<style>`, `<nav>`, `<header>`, `<footer>`, `<aside>`, cookie banners, modals, ads, and similar elements, then comparing the cleaned text length to the raw text length:
...
**Section Length** measures the average number of words between consecutive heading elements (H1-H6). Research shows pages using 120-180 words between headings receive 70% more AI citations:
...
**Answer Capsules** detects the "answer capsule" pattern from citation research: 72% of AI-cited content has a concise answer (under 200 characters) placed immediately after a question-framed H2. The check:
...
When someone asks ChatGPT "what is X?" or "how do I do Y?", the engine looks for content that directly answers that question. Pages that define terms, provide step-by-step instructions, and include Q/A sections give engines ready-made answer material. The answer capsule pattern is the single most predictive formatting trait for AI citations - research shows 72% of cited content uses it and answer-first formatting increased citations by 140%.
...
**Quoted Attribution** specifically detects the quote-with-attribution pattern that the Princeton GEO study found boosts visibility by 30-40%. Combines two checks:
...
**Content Freshness** goes beyond date presence to evaluate how recent the content actually is. Research shows 65% of AI crawler hits target content less than 1 year old, and freshness acts as a hard gate - stale content loses visibility regardless of quality.
...
analyzer/service.ts             orchestrates the full pipeline:
  ...
  |
  v
report/service.ts               renders output as pretty, json, or markdown
  |
  v
...
module/
  ...
  service.ts      Pure business logic (no try/catch)
  controller.ts   try/catch boundary, returns { data } | { error }
  constants.ts    Magic numbers, thresholds, config defaults
...
**Services** contain the actual logic. They take validated inputs, do work, and return typed results. They throw on failure - no defensive error handling.

**Controllers** wrap services in `try/catch`. They normalize any thrown error into a `NormalizedError` object with a code like `FETCH_ERROR`, `PARSE_ERROR`, `CONFIG_ERROR`, etc. The controller is the public API of each module.
...
```
audits/
  schema.ts              CategoryResult, FactorResult, AuditResult types
  service.ts             runAudits() - calls all 7 audit functions
  controller.ts          try/catch wrapper
  ...
  support/
    ...
    language.ts          NLP helpers (compromise entities, Flesch, syllables)
```

**`service.ts`** exports a single function `runAudits(page, fetchResult, domainSignals?)` that calls 7 private functions:

```
auditContentExtractability(page, fetchResult, rawData, domainSignals)
auditContentStructure(page, rawData)
auditAnswerability(page, rawData)
auditEntityClarity(page, rawData)
auditGroundingSignals(page, rawData)
auditAuthorityContext(page, rawData)
auditReadabilityForCompression(page, rawData)
```
...
3. Return a `CategoryResult` with `sumFactors(factors)` as the score
...
**`support/language.ts`** wraps the NLP dependencies and gap-analysis helpers:
